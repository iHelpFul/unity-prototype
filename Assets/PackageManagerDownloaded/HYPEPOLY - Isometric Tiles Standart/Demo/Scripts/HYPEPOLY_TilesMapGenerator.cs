...
public class HYPEPOLY_TilesMapGenerator : MonoBehaviour
{
    ...
    public EnableDisable generateAsCorridor = EnableDisable.Disabled;
    [Range(1, 25)]
    ...
    bool useCorridorModeThisGeneration;
    int currentGenerationGridSize;
    int currentGenerationCorridorMinX;
    int currentGenerationCorridorMaxX;
    float currentTileSpacing = 2f;
    void FixedUpdate()
    {
        if (TapToGenerate)
        {
                            ...
                            int _laddersChance)
    {
        ...
        currentGenerationGridSize = _mapSize;
        currentTileSpacing = tileSpacing > 0f ? tileSpacing : 2f;
        useCorridorModeThisGeneration = generateAsCorridor == EnableDisable.Enabled;

        if (useCorridorModeThisGeneration)
        {
            int clampedWidth = Mathf.Clamp(corridorWidth, 1, _mapSize);
            int halfWidth = clampedWidth / 2;
            int centerX = Mathf.FloorToInt((_mapSize - 1) / 2f);
            currentGenerationCorridorMinX = Mathf.Clamp(centerX - halfWidth, 0, _mapSize - 1);
            currentGenerationCorridorMaxX = Mathf.Clamp(currentGenerationCorridorMinX + clampedWidth - 1, 0, _mapSize - 1);

            int currentWidth = currentGenerationCorridorMaxX - currentGenerationCorridorMinX + 1;
            if (currentWidth < clampedWidth)
            {
                currentGenerationCorridorMinX = Mathf.Clamp(currentGenerationCorridorMaxX - clampedWidth + 1, 0, _mapSize - 1);
            }
        }
        else
        {
            currentGenerationCorridorMinX = 0;
            currentGenerationCorridorMaxX = _mapSize - 1;
        }
        ...
        if (_POIs == EnableDisable.Enabled)
        {
            for (int i = 0; i < _POIsCount; i++)
            {
                ...
                {
                    trys++;
                    int[] newPoi = new int[2] { GetRandomPlayableX(_xSize), Random.Range(0, _zSize) };
        ...
        if (_holesCount != GeneratorParametr.None)
        {
            ...
            {
                int hX = GetRandomPlayableX(_xSize);
        ...
        int roadsHeight = playableCellsCount > 0
            ? Mathf.CeilToInt(roadsSumHeights / playableCellsCount)
            : 0;
                    ...
                    if (!holesMap[i, a])
                    {
                        ...
                        if (needSpawn)
                        {
                            if (Random.Range(0, 100) < _laddersChance)
                            {
                                ...
                                ladder.transform.position = new Vector3(i * currentTileSpacing, _mapPos.y + (heightMap[a, i] * 2f), a * currentTileSpacing);
        ...
        for (int i = 0; i < _xSize; i++)
        {
            for (int a = 0; a < _zSize; a++)
            {
                ...
                float x = i * currentTileSpacing;
                float z = a * currentTileSpacing;
        ...
        if (_POIs == EnableDisable.Enabled)
        {
            for (int i = 0; i < pointsOfInterest.Count; i++)
            {
                ...
                poiObj.transform.position = new Vector3(zPos * currentTileSpacing, _mapPos.y + (heightMap[xPos, zPos] * 2f), xPos * currentTileSpacing);
                ...
                for (int a = 0; a < _zSize; a++)
                {
                    ...
                    if (roadsMap[i, a] && RoadIsNotPOI(i, a, pointsOfInterest))
                    {
                            ...
                            (up && down && right) || (up && left && right) || (up && left && down) || (right && left && down))
                        {
                            if (holesMap[a, i])
                            {
                                SpawnTile(i, a, (i * currentTileSpacing), (a * currentTileSpacing), mapPosition, map.transform, true, heightMap[a, i]);
                            }
                        }

                        if (spawned)
                        {
                            road.transform.position = new Vector3(i * currentTileSpacing, _mapPos.y + (heightMap[a, i] * 2f), a * currentTileSpacing);
    ...
    void AdditionalFilling(GeneratorParametr _additionalFilling, int sizeOfMap)
    {
        ...
        if (_additionalFilling != GeneratorParametr.None)
        {
            ...
            {
                Vector3 circleTreesPos = new Vector3(GetRandomWorldX(sizeOfMap), 15f, GetRandomWorldZ(sizeOfMap));
            ...
            {
                RaycastHit hit;
                Vector3 rayPos = new Vector3(GetRandomWorldX(sizeOfMap), 15f, GetRandomWorldZ(sizeOfMap));
                if (Physics.Raycast(rayPos, Vector3.down, out hit, Mathf.Infinity))
                {
                    if (hit.transform.name.Contains("Tile") && IsPosAvailableByDistance(hit.point, grassPoint, 0.25f) && isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap))
                    {
    ...
    {
        float spacing = currentTileSpacing > 0f ? currentTileSpacing : 2f;
        int x = Mathf.RoundToInt(posToCheck.x / spacing);
        int z = Mathf.RoundToInt(posToCheck.z / spacing);
    ...
    {
        if (!IsPlayableCell(_x, _z, curentMap.GetLength(0)))
            return curentMap;
    ...
    bool IsPlayableCell(int x, int z, int gridSize)
    {
        ...
        return x >= currentGenerationCorridorMinX && x <= currentGenerationCorridorMaxX;
    }

    int GetRandomPlayableX(int gridSize)
    {
        if (!useCorridorModeThisGeneration)
            return Random.Range(0, gridSize);

        int minX = Mathf.Clamp(currentGenerationCorridorMinX, 0, gridSize - 1);
        int maxX = Mathf.Clamp(currentGenerationCorridorMaxX, minX, gridSize - 1);
        return Random.Range(minX, maxX + 1);
    }

    float GetRandomWorldX(int gridSize)
    {
        float spacing = currentTileSpacing > 0f ? currentTileSpacing : 2f;
        int minX = useCorridorModeThisGeneration ? currentGenerationCorridorMinX : 0;
        int maxX = useCorridorModeThisGeneration ? currentGenerationCorridorMaxX : gridSize - 1;
        return Random.Range(minX * spacing, (maxX + 1) * spacing);
    }

    float GetRandomWorldZ(int gridSize)
    {
        float spacing = currentTileSpacing > 0f ? currentTileSpacing : 2f;
        return Random.Range(0f, gridSize * spacing);
    }

    void CenterGeneratedMap()
    {
        if (lastMap == null)
            return;

        float spacing = currentTileSpacing > 0f ? currentTileSpacing : 2f;
        float centerX = ((currentGenerationCorridorMinX + currentGenerationCorridorMaxX) * spacing) * 0.5f;
        float centerZ = ((currentGenerationGridSize - 1) * spacing) * 0.5f;

        for (int i = 0; i < lastMap.transform.childCount; i++)
        {
            Transform child = lastMap.transform.GetChild(i);
